// FROM WORKING VERSION: HLaEst2015BEP.mod // fix phi_psi=0.25, fix delta_B=0.4
...
phi_w, beta_pdf, 0.7, 0.05;              // 0.65 weight in borrowing constraint
Nu_h, beta_pdf, 0.75, 0.05;               // LTV ratio

//parameters to be estimated only

   //entrepreneurs
phi_k, beta_pdf, 0.75, 0.05;              // 0.85 weight in borrowing constraint
Nu_e, beta_pdf, 0.55, 0.05;               // LTV ratio

   //banks
phi_B, beta_pdf, 0.25, 0.05;            // pass through of equity price PCH in bank cap acc eqn
...
rho_psi, beta_pdf, 0.85, 0.02;            // AR(1) parameter for Equity shock
...
varobs pi_obs q_psi_obs y_obs l_h_obs l_e_obs d_obs i_d_obs i_h_obs i_e_obs;  // 

estimation(datafile=HL2015Estimation9shock, mh_nblocks=3, mh_replic=10000, mh_drop=0.5, mh_jscale=0.34, 
           mode_compute=0, mode_file=HLaEst2013FI4bNEW_mode, mode_check, bayesian_irf, irf=20, moments_varendo, conditional_variance_decomposition=[1:20],
